...
class KpmXapp(kpmframe.XappKpmFrame):
    ...
    def indication_callback(self, ind_hdr, ind_msg, meid):
        ...
        # Decoding sender_name safely
        sender_ptr = ind_hdr.data.kpm_ric_ind_hdr_format_1.sender_name
        if sender_ptr and sender_ptr.contents:
            my_string = bytes(np.ctypeslib.as_array(
            sender_ptr.contents.buf, 
            shape=(sender_ptr.contents.len,)
            ))
            sender_name = my_string.decode('utf-8', errors='ignore')
        else:
            sender_name = "unknown"
